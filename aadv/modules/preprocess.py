...
def xml_2csv(input_path, file_name):
    ...
    :return: None
    """
    tree = ET.ElementTree(file=input_path + '/data/' + file_name)
    root = tree.getroot()
    ...
    df.to_csv(input_path + '/data/' + 'AviationData.csv', index=None, header=True)
    return
...
def dataset():
    ...
    if not os.path.isfile(PATH + '/data/' + 'AviationData.csv'):
        xml_2csv(PATH, FILENAME)
    df = pd.read_csv(PATH + '/data/' + 'AviationData.csv')
    # df = drop_nan(df)
    df = elim_country(df)
    df = create_state(df)
    return df
